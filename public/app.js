...
const ui = {
    ...
    async doRoll(n, bid) {
        ...
        // --- AJOUT DES ÉTOILES ICI ---
        const starsContainer = document.createElement('div');
        starsContainer.className = 'stars-container';
        this.createStars(starsContainer, 100); // On appelle la fonction que tu as ajoutée
        overlay.appendChild(starsContainer);

        // Si 5★, on peut ajouter une classe pour des étoiles dorées via CSS
        if (data.obtainedCards.some(c => c.rarity === 5)) {
        ...
        overlay.innerHTML += `
            <div class="card-grid" style="display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 20px; width: 85%; max-width: 1200px;">
                ${data.obtainedCards.map((c, index) => `
                    <div class="card card-anim rarity-${c.rarity}" 
                        style="animation-delay: ${index * 0.15}s; margin: 0;">
                        <img src="${c.img}">
                        <p style="text-align: center; margin-top: 10px;">
                            ${c.name} <br> 
                            <span style="color: var(--accent)">${'★'.repeat(c.rarity)}</span>
                        </p>
                    </div>`).join('')}
            </div>
            <p class="fade-in-text" style="color: white; margin-top: 40px; font-style: italic; opacity: 0; animation: fadeIn 1s forwards; animation-delay: ${data.obtainedCards.length * 0.2}s;">
...
const ui = {
    ...
    async doRoll(n, bid) {
        ...
        // 2. On injecte les cartes à l'intérieur
        overlay.innerHTML = `
            <div class="card-grid" style="display : flex; justify-content: center; align-items: center; gap: 20px; width: 80%; max-width: 1200;">
                ${data.obtainedCards.map(c => `
                    <div class="card card-anim rarity-${c.rarity}">
                        <img src="${c.img}">
                        <p style="padding-left: 15px;">${c.name}   ${c.rarity}★</p>
                    </div>`).join('')}
            </div>
