...
const ui = {
    ...
    showVowPopup(message) {
        const popup = document.createElement('div');
        popup.className = 'vow-notification-popup';
        popup.innerHTML = `
            <div class="vow-popup-content">
                <p>${message}</p>
    ...
    async checkNotifications() {
        if (currentUser && currentUser.pendingNotification) {
            this.showVowPopup(currentUser.pendingNotification);
            
            // On efface la notification sur le serveur pour ne pas qu'elle revienne
            await fetch('/api/user/clear-notification', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: currentUser.id })
            });
            currentUser.pendingNotification = null;
        }
    },
    ...
    async loadTab(tab) {
        ...
        if (tab === 'Collection' || tab === 'Ma collection') {
            ...
            // 1. On filtre les cartes en fonction des filtres ET de la barre de recherche
            const filteredCards = globalData.cards.filter(c => {
                ...
                const matchesPersonnage = this.filters.search === "all" || c.name.includes(this.filters.search);
                
                let matchesOwned = true;
                if(this.filters.owned === "owned") matchesOwned = qty > 0;
                if(this.filters.owned === "missing") matchesOwned = qty === 0;

                return matchesQuery && matchesRarity && matchesPersonnage && matchesOwned;
            });

            area.innerHTML = `
                <div class="collection-header" style="display:flex; justify-content:space-between; align-items:center;">
                    <h2>Ma Collection (${filteredCards.length})</h2>
                    <button class="btn-tab" onclick="ui.openFilterPopup()">⚙️ Filtres</button>
                </div>
                <div style="margin-top:15px;">
                    <input type="text" id="card-search-bar" placeholder="Rechercher un personnage..." value="${this.searchQuery}"
                        style="width:100%; padding:12px; background:rgba(0,0,0,0.3); border:1px solid var(--border); border-radius:10px; color:white; outline:none;">
                </div>
                    ...
                    ${filteredCards.length > 0 ? filteredCards.map(c => {
                        ...
                        <div class="card ${isLocked ? 'locked' : ''} rarity-${c.rarity}">
                            ...
                            <div class="card-info" style="padding:10px; text-align:center;">
                                <strong style="font-size:0.8rem;">${c.name}</strong><br>
                                <span style="color:var(--rarity-${c.rarity})">${'★'.repeat(c.rarity)}</span>
                                ${qty > 1 ? `<div class="qty-badge">x${qty}</div>` : ''}
                                ${!isLocked ? `<br><button class="btn-avatar-mini" onclick="ui.setAvatar('${c.id}')">Avatar</button>` : ''}
                            </div>
                        </div>`;
                    }).join('') : `<p style="text-align:center; grid-column: 1/-1; opacity:0.5;">Aucun résultat trouvé</p>`}
                </div>`;

            // --- LA LOGIQUE DE FIX ---
            const sb = document.getElementById('card-search-bar');
            
            // On remet le focus sur la barre après le rendu
            sb.focus();
            // On place le curseur à la fin du texte
            sb.setSelectionRange(sb.value.length, sb.value.length);

            // On écoute l'événement 'input' (temps réel)
            sb.oninput = (e) => {
                this.searchQuery = e.target.value; // On met à jour la variable globale
                this.loadTab('Collection');        // On relance le rendu filtré
            };
        }
    },
