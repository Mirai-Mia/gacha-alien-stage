    ...
    async loadTab(tabName) {
        ...
        if (tabName === 'Bannières') {
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;">
                <h2 style="color:var(--accent); text-shadow: 0 0 10px var(--accent-glow);">⚡ Gestion des Flux de Vœux</h2>
                <button class="btn-save" onclick="adminLogic.createNewBanner()">+ Nouvelle Flux</button>
            </div>
                ...
                <div class="admin-banner-card" data-banner-id="${b.id}">
                    <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:15px;">
                        <div>
                            ...
                            <h3 style="margin: 10px 0 5px 0;">${b.id.toUpperCase()}</h3>
                        </div>
                    ...
                    <label style="font-size:0.8rem; color:var(--accent);">URL DE L'ILLUSTRATION</label>
                    <input type="text" class="banner-img-input" value="${b.image || ''}" 
                        style="width:100%; background:#000; border:1px solid var(--border); color:white; padding:10px; border-radius:5px; margin:5px 0 20px 0;">
                    
                    <details>
                        <summary style="cursor:pointer; color:var(--accent); margin-bottom:10px;">Sélection des cartes (${b.cards.length})</summary>
                        <div class="card-grid" style="grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); gap:10px;">
                            ${globalData.cards.map(c => `
                                <div class="card mini" style="font-size:0.7rem;">
                                    <img src="${c.img}" style="height:70px;">
                    ...
                    <button class="btn-roll" style="width:100%; margin-top:20px; background:var(--accent); color:#000;" 
                            onclick="adminLogic.updateBanner('${b.id}')">Sauvegarder les modifications</button>
                </div>
            `).join('')}`;
        }
