...
const ui = {
    ...
    filters: {
        rarity: "all",
        search: "all", // Gardé pour le filtre par liste du popup
        owned: "all"
    },
    ...
    async loadTab(tab) {
        ...
        if (tab === 'Collection' || tab === 'Ma collection') {
            ...
            const filteredCards = globalData.cards.filter(c => {
                ...
                // Filtre de la barre de recherche (Temps réel)
                ...
                const matchesNameList = this.filters.search === "all" || c.name.includes(this.filters.search);
                
                let matchesOwned = true;
                if(this.filters.owned === "owned") matchesOwned = qty > 0;
                if(this.filters.owned === "missing") matchesOwned = qty === 0;

                return matchesQuery && matchesRarity && matchesNameList && matchesOwned;
            });
            ...
            area.innerHTML = `
                <div class="collection-header">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px;">
                        ...
                        <button class="btn-tab" onclick="ui.openFilterPopup()">⚙️ Filtres Avancés</button>
                    </div>
                        ...
                        return `
                            <div class="card ${isLocked ? 'locked' : ''} rarity-${c.rarity}">
                                ...
                                <div class="card-info" style="padding:10px; text-align:center;">
                                    ...
                                    <span class="rarity-text" style="color:var(--rarity-${c.rarity})">${'★'.repeat(c.rarity)}</span>
                                    ${qty > 1 ? `<div class="qty-badge">x${qty}</div>` : ''}
                                </div>
                            </div>`;
                    }).join('') : `<p style="text-align:center; grid-column: 1/-1; opacity:0.5; margin-top:50px;">Aucune carte ne correspond à votre recherche.</p>`}
                </div>`;
